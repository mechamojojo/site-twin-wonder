...
export async function getCssbuyProductPreview(
  ...
  try {
      ...
      try {
        ...
        if (
          ...
          if (
            ...
            if (!capturedApiResponses.includes(d[0]))
        ...
        if (
          ...
          if (
            ...
            payload &&
            !capturedApiResponses.includes(payload)
          )
            capturedApiResponses.push(payload);
        }
      } catch {
        // ignore
      }
        ...
        for (const s of scripts) {
          ...
          if (idx !== -1) {
            ...
            while ((pm = propNameRe.exec(arrContent)) !== null) {
              ...
              const isColorByName =
                /颜色|色彩|color|cor|款式|estilo|modelo|model|style/i.test(
                  propName,
                );
              ...
              if (vals.length > 0) {
                const normImgs = imgs
                  ...
                  .map((u) =>
                    u
                      ? u.replace(
                          /_(\d{2,3})x(\d{2,3})(\.(jpg|jpeg|png|webp))?/gi,
                          "_200x200$3",
                        )
                      : "",
                  );
    ...
    function mergeFromApiResponses(apiList: unknown[]) {
      ...
      type SkuListType = Array<{
        properties?: string;
        price?: number;
        quantity?: number;
      }>;
      let mergedPropsList: Record<string, unknown> = {};
      const skuPropsByGroupName = new Map<
        string,
        { values: string[]; images: string[] }
      >();
      ...
      for (const api of apiList) {
        ...
        if (!sp) {
          const skuCore = findInObj(r, ["skuCore"]) as
            ...
            | unknown[]
            | undefined;
          if (skuCore && Array.isArray(skuCore)) sp = skuCore as SkuPropsType;
          else if (
            skuCore &&
        ...
        if (sp && Array.isArray(sp)) {
          for (const prop of sp) {
            const name =
              String(prop.propName ?? prop.prop ?? "").trim() || "Opção";
            const valuesArr = prop.values;
            if (!valuesArr || !Array.isArray(valuesArr)) continue;
            const existing = skuPropsByGroupName.get(name) ?? {
              values: [],
              images: [],
            };
      ...
      const bestPropsList =
        Object.keys(mergedPropsList).length > 0 ? mergedPropsList : null;
      const bestSkuProps: SkuPropsType = Array.from(
        skuPropsByGroupName.entries(),
      ).map(([name, { values, images }]) => ({
        propName: name,
      ...
      const bestSkuList: SkuListType = Array.from(
        skuByProps.values(),
      ) as SkuListType;
    ...
    const isColorLabel = (label: string) =>
      /^color|colour|颜色|款式|style|estilo|cor|model|modelo/i.test(
        label.trim(),
      );
    ...
    if (skuProps && Array.isArray(skuProps) && data.colorValues.length === 0) {
      for (const prop of skuProps) {
        ...
        if (!/style|color|colour|颜色|款式|estilo|model|modelo/.test(name))
          continue;
        const valuesArr = (
    ...
    if (skuProps && Array.isArray(skuProps)) {
      for (const prop of skuProps) {
        const name =
          String(
            (prop as { propName?: string }).propName ??
              (prop as { prop?: string }).prop ??
              "",
          ).trim() || "Opção";
        const valuesArr = (
          prop as { values?: Array<{ value?: string; image?: string }> }
    ...
    const isPackagingOrAccessory = (value: string): boolean => {
      ...
      return (
        /礼盒|包装|标配|盒装|收纳|配件|赠品|包装盒|外盒|内盒|盒子|箱/.test(
          value,
        ) ||
        /\b(box|packaging|gift\s*box|case\s*only|accessory|acessório|embalagem|caixa)\b/i.test(
          v,
        )
      );
    };
    const isQualityGradeGroup = (name: string): boolean =>
      /品质等级|quality\s*level|quality\s*grade|nível\s*de\s*qualidade|qualidade/i.test(
        name.trim(),
      );
    let colorValuesFiltered = data.colorValues.filter(
      (v) => !isPackagingOrAccessory(v),
    );
    ...
    if (data.colorValues.length > 0 && data.sizeValues.length > 0) {
      if (tooManyNumericOptions(data.sizeValues)) {
        ...
        allNumericSizes(data.sizeValues) &&
        hasLowOptionIds(data.sizeValues)
      ) {
        data.sizeValues = [];
      }
    }
    ...
    if (
      ...
      for (const sku of skuList) {
        ...
        if (colorVal && !(colorVal in priceByValue))
          priceByValue[colorVal] = price;
      }
    ...
    const domGroups = fromDom?.length
      ? fromDom.map((g) => ({
          name: g.name,
          values: g.values,
          images: g.images ?? [],
      ...
      : [];
    const totalFromApi = optionGroupsFromApi.reduce(
      (s, g) => s + g.values.length,
      0,
    );
    ...
    if (propsList && typeof propsList === "object") {
      ...
      for (const [propKey, v] of plEntries) {
        ...
        if (typeof v === "string") {
          ...
          img =
            ...
            (propsImg as Record<string, string> | undefined)?.[propKey];
        }
        const colonIdx = raw.indexOf(":");
        const label = colonIdx >= 0 ? raw.slice(0, colonIdx).trim() : "";
        const val = colonIdx >= 0 ? raw.slice(colonIdx + 1).trim() : raw.trim();
      ...
      if (sizeValues.length > 0) {
        const numericSizes = sizeValues.filter((v) =>
          /^\d+(\.5)?$/.test(v.trim()),
        );
        const otherSizes = sizeValues.filter(
          (v) => !/^\d+(\.5)?$/.test(v.trim()),
        );
    ...
    if (
      ...
      for (const sku of skuList) {
        ...
        const qty =
          typeof (sku as { quantity?: number }).quantity === "number"
            ? (sku as { quantity: number }).quantity
            : parseInt(
                String(
                  (sku as Record<string, unknown>).quantity ??
        ...
        if (styleVal && sizeVal) {
          if (!inventoryByColorAndValue[styleVal])
            inventoryByColorAndValue[styleVal] = {};
          inventoryByColorAndValue[styleVal][sizeVal] =
            (inventoryByColorAndValue[styleVal][sizeVal] ?? 0) + qty;
        }
      }
      if (Object.keys(inventoryByColorAndValue).length > 0)
        sizeGroupFromProps.inventoryByColorAndValue = inventoryByColorAndValue;
    }
    const totalFromPropsList = optionGroupsFromPropsList.reduce(
      (s, g) => s + g.values.length,
      0,
    );
    ...
    if (
      totalFromPropsList > totalFromApi &&
      totalFromPropsList > totalFromDom &&
      optionGroupsFromPropsList.length > 0
    ) {
      ...
      if (
        colorGroup.priceByValue &&
        Object.keys(colorGroup.priceByValue).length > 0
      ) {
        const firstWithValues = optionGroups.find((g) => g.values.length > 0);
        if (firstWithValues)
          firstWithValues.priceByValue = colorGroup.priceByValue;
      }
    ...
    if (!hasQualityGroup) {
      const fromApi = optionGroupsFromApi.find((g) =>
        isQualityGradeGroup(g.name),
      );
      ...
      if (qualityGroup && qualityGroup.values.length > 0) {
        optionGroups.push({
          name: "Quality grade",
          values: qualityGroup.values,
          images: qualityGroup.images ?? [],
        });
      }
    }
    ...
    if (sizeGroupEmpty) {
      for (const g of optionGroups) {
        ...
        g.values.forEach((val, i) => {
          if (isSizeLike(val)) sizeLikeIndices.push(i);
          else nonSizeLikeIndices.push(i);
        });
        const hasBoth =
          sizeLikeIndices.length > 0 && nonSizeLikeIndices.length > 0;
        if (!hasBoth) continue;
        const nonSizeValues = nonSizeLikeIndices.map((i) => g.values[i]);
        const nonSizeImages = nonSizeLikeIndices.map(
          (i) => g.images?.[i] ?? "",
        );
        const sizeValues = sizeLikeIndices.map((i) => g.values[i]);
        const sizeImages = sizeLikeIndices.map((i) => g.images?.[i] ?? "");
        g.values = nonSizeValues;
        g.images = nonSizeImages;
        if (sizeGroupExisting) {
          sizeGroupExisting.values = sizeValues;
          sizeGroupExisting.images = sizeImages;
        } else {
          optionGroups.push({
            name: "Tamanho",
            values: sizeValues,
    ...
    for (const g of optionGroups) {
      ...
      if (
        g.inventoryByColorAndValue &&
        typeof g.inventoryByColorAndValue === "object"
      ) {
        const next: Record<string, Record<string, number>> = {};
        for (const [colorKey, byVal] of Object.entries(
          g.inventoryByColorAndValue,
        )) {
