...
const isSizeGroup = (name: string, values?: string[]) => {
  if (/cor|color|颜色|色彩|estilo|款式|modelo|型号|style/i.test(name))
    return false;
  if (values && values.length > 0) {
    const allLookLikeSizes = values.every(
      (v) =>
...
function resolveHeroImageForCartLine(
  ...
  const imageGroup = productPreview.optionGroups?.find(
    (g) =>
      ...
      (g.displayAsImages === true ||
        (g.displayAsImages !== false && g.images?.some(Boolean))),
  );
  const selectedVal = imageGroup
    ? selectedOptionByGroup[imageGroup.name]
    : undefined;
  const imgIdx =
    selectedVal && imageGroup ? imageGroup.values.indexOf(selectedVal) : -1;
  const optionImg =
    imageGroup && imgIdx >= 0 ? imageGroup.images?.[imgIdx] : null;
  if (optionImg && String(optionImg).trim() !== "")
    return String(optionImg).trim();
  ...
  const fromGroup = imageGroup?.images?.find(
    (u) => u && String(u).trim() !== "",
  );
...
const Order = () => {
  ...
  const [productPreviewError, setProductPreviewError] = useState<string | null>(
    null,
  );
    ...
    async (forceRefresh = false) => {
      ...
      try {
        ...
        const res = await fetch(
          apiUrl(
            `/api/product/preview?url=${encodeURIComponent(url)}${nocache}`,
          ),
        );
        if (!res.ok) {
  ...
  useEffect(() => {
    const fetchPreview = async () => {
      ...
      try {
        ...
        const res = await fetch(
          apiUrl(`/api/price/preview?url=${encodeURIComponent(url)}`),
        );
        if (!res.ok) {
          const data = await res.json().catch(() => ({}));
          throw new Error(
            data.error || "Não foi possível obter o preço aproximado.",
          );
        }
        const data = await res.json();
        setPreview({
  ...
  useEffect(() => {
    ...
    const priceParam =
      effectivePriceCny !== productPreview?.priceCny
        ...
        : "";
    fetch(
      apiUrl(`/api/price/preview?url=${encodeURIComponent(url)}${priceParam}`),
    )
  ...
  const saveSnapshotForAll = useCallback(async () => {
    ...
    try {
      ...
      if (res.status === 401) {
        ...
        toast.error(
          "Sessão expirada. Faça login em /admin (use o mesmo domínio: com ou sem www).",
        );
        return;
      }
      if (!res.ok) {
      ...
      toast.success(
        "Página salva. Todos os usuários verão esta mesma página ao abrir este link.",
      );
    } catch (e) {
      ...
      <div className="min-h-screen bg-background">
        ...
        <main className="container mx-auto px-4 py-20 text-center">
          <p className="text-muted-foreground mb-6">
            Nenhum produto selecionado. Informe o link do produto na página
            inicial ou pesquise por palavras-chave.
          </p>
          <Link
            to="/"
        ...
        <main className="container mx-auto px-4 py-20 max-w-lg mx-auto">
          <div className="rounded-2xl border border-amber-200 bg-amber-50/50 dark:bg-amber-950/20 dark:border-amber-800 p-8 text-center">
            ...
            <p className="text-muted-foreground mb-4">
              O link informado não é válido ou não é de um marketplace
              suportado. Confirme se o link está correto e se é de Taobao, 1688,
              Weidian, TMALL, JD.com ou outras lojas chinesas.
            </p>
            <p className="text-sm text-muted-foreground mb-6">
              Se você queria pesquisar produtos, use palavras-chave como
              "bolsa", "tenis" ou "celular" na barra de pesquisa.
            </p>
            ...
            <div className="rounded-2xl border border-border bg-card overflow-hidden shadow-card sticky top-[4.5rem]">
                ...
                <div className="animate-pulse">
                  ...
                  <div className="p-3 flex gap-2 border-t border-border">
                    {[1, 2, 3, 4, 5].map((i) => (
                      <div
                        key={i}
                        className="w-16 h-16 sm:w-20 sm:h-20 rounded bg-muted/60 shrink-0"
                      />
                    ))}
                  </div>
                </div>
              ...
              {!productPreviewLoading &&
                ...
                (productPreview.images.length > 0 ||
                  ...
                  (productPreview.specs?.length ?? 0) > 0 ||
                  productPreview.description) && (
                  <>
                    <div className="bg-[#f5f5f5] overflow-hidden">
                      {productPreview.images.length > 0 ||
                      productPreview.optionGroups?.some(
                        (g) =>
                          (g.displayAsImages === true ||
                        ...
                        <>
                          ...
                          <div className="p-3 flex gap-2 overflow-x-auto border-t border-[#e8e8e8] bg-white">
                            {(() => {
                              ...
                              if (
                                imageGroup &&
                                (imageGroup.displayAsImages === true ||
                                  imageGroup.images?.some(Boolean))
                              ) {
                                const withThumbs = imageGroup.values
                                  .map((value, i) => ({
                                    value,
                                    i,
                                    thumb: imageGroup.images?.[i],
                                  }))
                                  .filter(
                                    ({ thumb }) =>
                    ...
                    <div className="p-3 border-t border-border flex items-center justify-between flex-wrap gap-2">
                      ...
                      <div className="flex items-center gap-3">
                        {isAdmin && (
                          ...
                          >
                            <Save
                              className={`w-3.5 h-3.5 ${saveSnapshotLoading ? "animate-pulse" : ""}`}
                            />
                            {saveSnapshotLoading
                              ? "Salvando..."
                              : "Salvar para todos"}
                        ...
                        >
                          <RefreshCw
                            className={`w-3.5 h-3.5 ${productPreviewLoading ? "animate-spin" : ""}`}
                          />
                          Atualizar
                        </button>
                        <a
                          ...
                          target="_blank"
                          rel="noreferrer"
                          className="text-xs text-china-red font-medium hover:underline inline-flex items-center gap-1"
                        >
                ...
                <>
                      ...
                      <>
                        <div className="flex flex-wrap items-baseline gap-2">
                          ...
                          <span
                            ...
                            title="Atualizado pela ComprasChina"
                          >
                  ...
                  <div className="rounded-lg bg-muted/40 border border-border p-3">
                    ...
                    <p className="text-xs text-muted-foreground">
                      Enviamos este produto até você no Brasil. O frete é
                      calculado no carrinho conforme seu CEP.
                    </p>
                    <Link
                      to="/#shipping"
                      className="text-xs text-china-red font-medium hover:underline mt-1 inline-block"
                    >
                      Ver opções de frete →
                    </Link>
                  </div>
                  ...
                  <div className="space-y-4">
                    ...
                    productPreview.optionGroups.length > 0 ? (
                      <>
                        {productPreview.optionGroups.map((group, gIdx) =>
                          isSizeGroup(group.name, group.values) ? (
                            ...
                            <div key={gIdx}>
                              ...
                              <ul className="space-y-3">
                                {group.values.map((value) => {
                                  ...
                                  const colorGroup =
                                    productPreview.optionGroups.find((g) =>
                                      /cor|color|estilo|style/i.test(g.name),
                                    );
                                  const selectedColor = colorGroup
                                    ? selectedOptionByGroup[colorGroup.name]
                                    : "";
                                  const stock =
                                    group.inventoryByColorAndValue?.[
                                      selectedColor
                                    ]?.[value] ??
                            ...
                            <div key={gIdx}>
                              ...
                              {group.displayAsImages === true ||
                              (group.displayAsImages !== false &&
                                ...
                                <div className="grid grid-cols-4 sm:grid-cols-5 lg:grid-cols-6 gap-2">
                                  {group.values
                                    ...
                                    .map(({ value, i, thumb }) => {
                                      const isSelected =
                                        selectedOptionByGroup[group.name] ===
                                        value;
                                      return (
                                        <button
                                          key={i}
                                          ...
                                          className={`flex flex-col items-center gap-1 p-2 sm:p-1.5 rounded-lg border-2 overflow-hidden transition-colors min-h-[52px] sm:min-h-0 ${
                                            isSelected
                                              ? "border-china-red ring-1 ring-china-red bg-china-red/5"
                                    ...
                                    return (
                                      <button
                                        ...
                                        className={`touch-target min-h-[44px] px-4 py-2.5 rounded-lg border-2 text-sm font-medium transition-colors ${
                                          isSelected
                                            ? "border-china-red bg-china-red/5 text-foreground"
                                            : "border-border bg-muted/30 hover:border-muted-foreground/50 text-foreground"
                                        }`}
                                        title={value}
                                      >
                                        {value}
                        ...
                        {!productPreview.optionGroups.some((g) =>
                          isSizeGroup(g.name, g.values),
                        ) && (
                          <div>
                      ...
                      <>
                          ...
                          <div>
                            ...
                            <div className="grid grid-cols-4 sm:grid-cols-5 lg:grid-cols-6 gap-2">
                              {productPreview.variants.color.map((c, i) => {
                                ...
                                const thumb =
                                  productPreview.variants?.colorImages?.[i] ??
                                  productPreview.images?.[i];
                                return (
                                  <button
                                    key={i}
                                    type="button"
                        ...
                        {productPreview.variants.size?.length ? (
                          <div>
                            <p className="text-xs font-semibold text-muted-foreground mb-2">
                              Tamanho (quantidade por tamanho):
                            </p>
                            <ul className="space-y-3">
                              {productPreview.variants.size.map((value) => {
                                ...
                                const selectedColor = colorGroup
                                  ...
                                  : "";
                                const stock =
                                  sizeGroup?.inventoryByColorAndValue?.[
                                    selectedColor
                                  ]?.[value] ??
                    ...
                    <details
                      ...
                      className={`rounded-lg border overflow-hidden transition-colors ${
                        disclaimerError
                          ? "border-amber-500 bg-amber-50/50 dark:bg-amber-950/20 dark:border-amber-500/50"
                    ...
                    >
                      ...
                      <div className="p-3 pt-0 text-[11px] text-muted-foreground space-y-2">
                        <p className="leading-relaxed">
                          Os produtos são vendidos por lojas nos marketplaces
                          chineses (Taobao, 1688, Weidian etc.). A ComprasChina
                          realiza a compra em seu nome e cuida de todo o envio
                          até o Brasil. A qualidade e as especificações dependem
                          do vendedor na plataforma de origem; verifique as
                          informações antes de finalizar.
                        </p>
                        <label
                          className="flex items-start gap-2 cursor-pointer"
                          onClick={() => setDisclaimerError(false)}
                        >
                          <input
                            type="checkbox"
                          ...
                          />
                          <span>
                            Li e concordo com o aviso e os termos de serviço.
                          </span>
                        </label>
                        {disclaimerError && (
                    ...
                    <div className="flex flex-col gap-2 pt-1">
                      {(() => {
                        const hasQuantityBySize = Object.values(
                          quantityBySize,
                        ).some((q) => q > 0);
                        const effectiveQuantity = hasQuantityBySize
                          ? Object.values(quantityBySize).reduce(
                              (a, b) => a + b,
                              0,
                            )
                          : quantity;
                        const sizeBreakdown = Object.entries(quantityBySize)
                          ...
                          .map(([s, q]) => `${s} x${q}`)
                          .join(", ");
                        const colorGroup = productPreview?.optionGroups?.find(
                          (g) =>
                            /^(Cor|Color|Style|Estilo|颜色|款式|Cor \/ Estilo)$/i.test(
                              g.name,
                            ),
                        );
                        ...
                        const otherOptions = Object.entries(
                          ...
                          .map(([k, v]) => `${k}: ${v}`)
                          .join("; ");
                        const colorPart = selectedColorValue
                          ? `Cor: ${selectedColorValue}`
                          : "";
                        const optsAndSize = productPreview?.optionGroups?.length
                        ...
                        const supportsKeepBox =
                          categorySupportsKeepBox(productCategory);

                        const handleAddToCart = () => {
                          if (!url) return;
                          if (!disclaimerAccepted) {
                            setDisclaimerError(true);
                            disclaimerDetailsRef.current?.setAttribute(
                              "open",
                              "",
                            );
                            disclaimerDetailsRef.current?.scrollIntoView({
                              behavior: "smooth",
                              block: "center",
                            });
                            return;
                    ...
                    <p className="text-[11px] text-muted-foreground text-center pt-2 border-t border-border">
                      ...
                      <a
                        ...
                        target="_blank"
                        rel="noreferrer"
                        className="text-china-red hover:underline inline-flex items-center gap-0.5"
                      >
                  ...
                  {(productPreview?.specs?.length ||
                    productPreview?.description) && (
                    <details className="rounded-lg border border-border overflow-hidden">
                      <summary className="px-3 py-2 text-xs font-semibold text-muted-foreground bg-muted/30 cursor-pointer">
                        Detalhes do item
                      </summary>
